...
\paragraph{Riproduzione}\hfill
Si utilizza il metodo di selezione dei centroidi casuale congiunto all'algoritmo di routing Nearest Neoghbourn, già presentato nella sezione precedente.
...
def GeneticAlgorithm():
	...
	for i in range(n_population):
		...
		if(GAPassignementRand != -1):
			chromosome = FisherJaikumar_NearestNeightbourn(graphToSolve,GAPassignementRand,K_clusterRand) 
	...
	while(era<Eras):
		for k in range(len(population)/2):
			...
			children,fittingCrossover = sol.Crossover(winner1,winner2,graphToSolve)\\
