	...
	while(era<Eras):
		...
		for k in range(len(population)/2):
			...
			if (np.random.randint(1,100) <= mutationRate):
				...
				mutant = Mutation(mutantChild[1],graphToSolve,1)
				fittingMutation = sum([m.getCost() for m in mutant])
				if(fittingMutation < mutantChild[0]):
					if(SearchaAndCompleteSequence(mutant,graphToSolve)):
						print("Invalid! " +str(fittingCrossover))
					else:
...
Un altra mutazione messa in atto è quella presente nel blocco da riga 54 a riga 65, il blocco di codice è simile a quello precedente se non per la riga 58 dove viene chiamato il metodo \texttt{Mutation}
