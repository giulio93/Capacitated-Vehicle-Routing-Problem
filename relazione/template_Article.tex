...
Altrimenti,se la coppia di clienti non corrisponde con nessuna \emph{route},allora una nuova \emph{route} viene create, questa nell'approccio sequenziale invece verrebbe scartata.
...
def Clarke&Wright_Sequenziale:
	...
	while((*$ \sum_{(r) \in R} \sum_{(v) \in V} ( v \in r) \neq  \ \mid V \mid $*) and len(savings)>0):
		pair = savings.pop()
		if(pair not served yet):
			route = Route.create(pair)
			
			for save in savings:
				if(i or j in save are in route):
					route.join(i or j)
					savings.remove(save)
					savings.updateDistance(i,j)
					restart from top of savings
