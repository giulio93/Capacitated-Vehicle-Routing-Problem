...
Il programma richiede \textbf{python3}.
Il codice si struttura nei seguenti file principali:
\begin{itemize}
\item  \texttt{main.py} Il metodo principale da dover far partire il programma, da riga di comanda una volta posizionati all'interno della cartella digitare \texttt{python3 main.py}. Al suo interno la stringa \texttt{benchmark}  seleziona l'insieme di istanze sulle quali applicare gli algoritmi elencati. I possibili valori sono \texttt{cvrp} le istanze fornite dal Prof. Bresolin e \texttt{avrp} le istanze dello studio \cite{studio}.

\item  \texttt{parser.py} All'interno si trovano tutti i metodi responsabili per la traduzione in grafo delel istanze TSPLIB95 e A-VRP.

\item \texttt{cvrpGraph} La classe che contiene il grafo utilizzato per rappresentare le istanze e i metodi per estrarre le informazioni.

\item \texttt{route.py} La classe che descrive una \emph{route} con i metodi per modificare e aggiungere i clienti. 

\item \texttt{solver.py} La classe principale che contiene tutti i metodi elencati, utilizzati per risolvere le istanze.


\item \texttt{CompareResult.txt} Un file che contiene la cronologia di tutte le istanze risolte dal programma.

\end{itemize}

Le cartelle presenti contengono le soluzioni delle istanze per ogni tipo di algoritmo applicato, la cartella \emph{relazione} contiene questo stesso elaborato. Nella cartela è presente anche lo studio \cite{studio}.
